...
use super::*;
    ...
    fn h5_parse_rust_extracts_symbols() {
        ...
        assert!(names.contains(&"Color"), "expected Color in symbols, got {:?}", names);
        assert!(names.contains(&"bar"), "expected bar in symbols, got {:?}", names);
        // 验证 SymbolKind
        assert!(result.symbols.iter().any(|s| s.kind == SymbolKind::Struct && s.name == "Foo"));
        assert!(result.symbols.iter().any(|s| s.kind == SymbolKind::Enum && s.name == "Color"));
        assert!(result.symbols.iter().any(|s| s.kind == SymbolKind::Function && s.name == "bar"));
    }
    ...
    fn h5_parse_typescript_extracts_symbols() {
        ...
        assert!(!result.has_errors);
        let names: Vec<&str> = result.symbols.iter().map(|s| s.name.as_str()).collect();
        assert!(names.contains(&"User"), "expected User in symbols, got {:?}", names);
        assert!(names.contains(&"getUser"), "expected getUser in symbols, got {:?}", names);
        assert!(result.symbols.iter().any(|s| s.kind == SymbolKind::Interface && s.name == "User"));
        assert!(result.symbols.iter().any(|s| s.kind == SymbolKind::Function && s.name == "getUser"));
    }

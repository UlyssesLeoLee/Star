...
pub struct GitHubProvider { pub token: String, pub base_url: String }

impl GitHubProvider {
    pub fn from_env() -> Result<Self, ProviderError> {
        let token = std::env::var("GITHUB_TOKEN").map_err(|_| ProviderError::Auth("GITHUB_TOKEN unset".into()))?;
        Ok(Self { token, base_url: "https://api.github.com".into() })
    }
}
...
#[cfg(test)] mod tests { use super::*; #[test] fn github_from_env() { std::env::set_var("GITHUB_TOKEN","x"); assert!(GitHubProvider::from_env().is_ok()); } }

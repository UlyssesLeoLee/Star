...
impl Provider for LocalProvider {
    ...
    async fn get_repo(&self, _owner: &str, _name: &str) -> Result<Repo, ProviderError> { Err(ProviderError::NotFound("n/i".into())) }
    async fn list_branches(&self, _owner: &str, _repo: &str) -> Result<Vec<Branch>, ProviderError> {
        let out = Command::new("git").args(["branch"]).output().await.map_err(|e| ProviderError::Network(e.to_string()))?;
        Ok(String::from_utf8_lossy(&out.stdout).lines().map(|l| Branch { name: l.trim().replace("* ", ""), sha: "".into(), protected: false }).collect())
    }
    ...
    async fn create_pull_request(&self, _owner: &str, _repo: &str, _args: CreatePrArgs) -> Result<PullRequest, ProviderError> { Err(ProviderError::Other("local git has no PR".into())) }
...
#[cfg(test)] mod tests { use super::*; #[tokio::test] async fn local_list_branches() { let p = LocalProvider; let r = p.list_branches("","").await; assert!(r.is_ok()); } }

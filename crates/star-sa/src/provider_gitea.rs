...
//! Gitea Cloud Provider — per spec/vcs/05 §2 (Phase F 倾向 Gitea)
//! 真实 API 接入（Phase F 接入真实网络，env:GITEA_TOKEN 读取）

use super::*;
use async_trait::async_trait;

pub struct GiteaProvider { pub token: String, pub base_url: String }

impl GiteaProvider {
    pub fn from_env() -> Result<Self, ProviderError> {
        let token = std::env::var("GITEA_TOKEN").map_err(|_| ProviderError::Auth("GITEA_TOKEN unset".into()))?;
        Ok(Self { token, base_url: "https://gitea.com/api/v1".into() })
    }
}
...
#[cfg(test)] mod tests { use super::*; #[test] fn gitea_from_env() { std::env::set_var("GITEA_TOKEN","x"); assert!(GiteaProvider::from_env().is_ok()); } }

...
impl BitbucketProvider {
    pub fn from_env() -> Result<Self, ProviderError> {
        let token = std::env::var("BITBUCKET_APP_PASSWORD")
            .map_err(|_| ProviderError::Auth("BITBUCKET_APP_PASSWORD unset".into()))?;
        Ok(Self {
            token,
...
mod tests {
    use super::*;
    #[test]
    fn bitbucket_from_env() {
        std::env::set_var("BITBUCKET_APP_PASSWORD", "x");
        assert!(BitbucketProvider::from_env().is_ok());
    }
}

...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrioritySlice { pub key: String, pub count: f64, pub percentage: f64 }
...
pub async fn generate(
    ...
    let raw = vec![("highest", 3.0), ("high", 12.0), ("medium", 35.0), ("low", 28.0), ("lowest", 7.0)];
    let total: f64 = raw.iter().map(|(_, c)| c).sum();
    let slices: Vec<PrioritySlice> = raw.iter().map(|(k, c)| PrioritySlice {
        key: k.to_string(),
        count: *c,
        percentage: c / total,
    }).collect();

    let data = PriorityDistData { slices: slices.clone(), total, status_filter: "open".to_string() };
...
mod tests {
    ...
    fn test_priority_slice_serde() {
        let s = PrioritySlice { key: "high".into(), count: 5.0, percentage: 0.5 };

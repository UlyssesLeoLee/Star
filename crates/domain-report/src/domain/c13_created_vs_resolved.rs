...
pub async fn generate(
    ...
    // 阶段 2 简化: mock 30 天
    let days = 30;
    let series: Vec<DayStat> = (0..days).map(|i| {
    ...
    let trend = if net > 5.0 { "growing" } else if net < -5.0 { "shrinking" } else { "stable" };
    ...
    Ok(ReportResult {
        ...
        summary: ReportSummary {
            ...
            anomalies: vec![],
            meta: serde_json::to_value(&data.summary).map_err(|e| ReportError::Internal(e.to_string()))?,
        },
...
mod tests {
    ...
    fn test_cvr_data_structure() {
        let s = DayStat { day: "2026-09-01".into(), created: 5.0, resolved: 3.0 };
    ...
    fn test_cvr_backlog_trend() {
        let summary = CvrSummary { total_created: 100.0, total_resolved: 90.0, net_change: 10.0, backlog_trend: "growing".into() };
        assert_eq!(summary.backlog_trend, "growing");
    }

    #[test]
    fn test_cvr_serialization() {
        let d = CvrData { series: vec![], summary: CvrSummary { total_created: 0.0, total_resolved: 0.0, net_change: 0.0, backlog_trend: "stable".into() }, time_granularity: "day".into() };

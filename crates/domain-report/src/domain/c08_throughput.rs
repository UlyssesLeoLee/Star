...
pub async fn generate(
    ...
    // mock: 14 weeks data
    let series: Vec<BucketCount> = (0..14)
        .map(|i| BucketCount {
            bucket: (Utc::now() - Duration::days((14 - i) * 7))
                .format("%Y-W%V")
                .to_string(),
    ...
    let moving_avg: Vec<BucketAvg> = counts
        .windows(3)
        .enumerate()
        .map(|(i, w)| BucketAvg {
    ...
    let std_dev =
        (counts.iter().map(|c: &f64| (c - avg).powi(2)).sum::<f64>() / counts.len() as f64).sqrt();
    ...
    Ok(ReportResult {
        ...
        summary: ReportSummary {
            ...
            trend: Trend::Flat,
            anomalies: vec![],
            meta: serde_json::to_value(&data.stats)

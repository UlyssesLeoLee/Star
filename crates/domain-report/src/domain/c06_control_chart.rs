...
pub async fn generate(
    ...
    let cycle_times: Vec<f64> = (0..20).map(|i| 3.0 + (i as f64 * 0.4).sin() * 1.5 + 0.3 * i as f64).collect();
    ...
    Ok(ReportResult {
        ...
        points: data.data_points.iter().enumerate().map(|(i, p)| ReportPoint {
            label: p.key.clone(),
...
pub fn detect_anomalies(cycle_times: &[f64]) -> (Vec<ControlPoint>, ControlStats) {
    ...
    if n < 10 {
        ...
        let points: Vec<ControlPoint> = (0..n).map(|i| ControlPoint {
        ...
        let stats = ControlStats {
            ...
            mean: if n > 0 { cycle_times.iter().sum::<f64>() / n as f64 } else { 0.0 },
            std_dev: 0.0,
        };
        return (points, stats);
    }
    ...
    let points: Vec<ControlPoint> = cycle_times.iter().enumerate().map(|(i, &x)| {
    ...
    let stats = ControlStats { median, p70, p85, p95, mean, std_dev };
    (points, stats)
}

fn build_reference_lines(stats: &ControlStats) -> Vec<RefLine> {
    let mut lines = vec![
        RefLine { y_value: stats.median, label: "Median".into(), style: "solid".into() },
        RefLine { y_value: stats.p70, label: "70%".into(), style: "dashed".into() },
        RefLine { y_value: stats.p85, label: "85%".into(), style: "dashed".into() },
        RefLine { y_value: stats.p95, label: "95%".into(), style: "dashed".into() },
    ];
    if stats.std_dev > 0.0 {
        let three_sigma = 3.0 * stats.std_dev;
        lines.push(RefLine { y_value: stats.mean + three_sigma, label: "+3σ".into(), style: "dotted".into() });
        lines.push(RefLine { y_value: (stats.mean - three_sigma).max(0.0), label: "-3σ".into(), style: "dotted".into() });
    }
    lines
}
...
mod tests {
    ...
    fn test_reference_lines_with_zero_std() {
        let stats = ControlStats { median: 5.0, p70: 6.0, p85: 7.0, p95: 8.0, mean: 5.0, std_dev: 0.0 };

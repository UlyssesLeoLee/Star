...
impl RiskError {
    ...
    pub fn not_found(worktree_id: uuid::Uuid, trace_id: impl Into<String>) -> Self {
        Self::new("RISK.NOT_FOUND", format!("No risks found for Worktree {}", worktree_id), trace_id)
    }

    /// RISK.GIT_FAIL
    pub fn git_fail(source: git_adapter::GitError) -> Self {
        Self::new("RISK.GIT_FAIL", source.message.clone(), source.trace_id.clone())
    }

    /// RISK.GRAPH_FAIL
    pub fn graph_fail(source: graph_core::GraphError) -> Self {
        Self::new("RISK.GRAPH_FAIL", format!("{:?}", source), "n/a".to_string())
    }

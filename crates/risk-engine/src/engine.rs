...
impl RiskEngine for InMemoryRiskEngine {
    async fn detect_risks(&self, _worktree_id: WorktreeId) -> Result<Vec<Risk>, RiskError> {
        ...
        let mut out: Vec<Risk> = guard
            ...
            .cloned()
            .collect();
        out.sort_by(|a, b| b.risk_score.value().partial_cmp(&a.risk_score.value()).unwrap_or(std::cmp::Ordering::Equal));
        Ok(out)
    }
...
mod tests {
    ...
    async fn risk_threshold_07_04_classification() {
        ...
        let risks = engine.detect_with_v1(input).await;
        let stale = risks.iter().find(|r| r.risk_type == RiskType::Stale).unwrap();
        assert_eq!(stale.risk_score.value(), 1.0, "extreme stale clamped to 1.0");

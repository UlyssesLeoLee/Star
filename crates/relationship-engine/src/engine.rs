...
use graph_core::repository::GraphRepository;
use graph_core::types::{RepoId, WorktreeId};

use crate::edge_ops::{
    validate_kind, EdgeCreateRequest, HighRiskKind, HighRiskMetadata,
};
...
pub struct InMemoryRelationshipEngine {
    ...
    edges: Arc<RwLock<HashMap<(EdgeKind, Uuid, Uuid), EdgePayload>>>,
...
impl RelationshipEngine for InMemoryRelationshipEngine {
    async fn create_edge(&self, req: EdgeCreateRequest) -> Result<EdgePayload, RelationshipError> {
        ...
        let edge = match req.kind {
            EdgeKind::DerivedFrom => EdgePayload::DerivedFrom(
                graph_core::edge::DerivedFromEdge {
                    from,
                    to,
                    created_at: now,
                    merge_base_sha: None,
            ...
            EdgeKind::ConflictsWith => EdgePayload::ConflictsWith(
                graph_core::edge::ConflictsWithEdge {
                    from,
                    to,
                    risk_score: graph_core::types::RiskScore::new(
                        req.high_risk_metadata.as_ref().map(|m| m.risk_score).unwrap_or(0.0),
                    ),
                    shared_files: req
                        .high_risk_metadata
                        .as_ref()
            ...
            EdgeKind::OverlapsWith => EdgePayload::OverlapsWith(
                graph_core::edge::OverlapsWithEdge {
                    from,
                    to,
                    overlap_score: req
                        .high_risk_metadata
            ...
            EdgeKind::DependsOn => EdgePayload::DependsOn(
                graph_core::edge::DependsOnEdge {
                    from,
                    to,
            ...
            EdgeKind::Supersedes => EdgePayload::Supersedes(
                graph_core::edge::SupersedesEdge {
                    from,
                    to,
                    created_at: now,
                    reason: req
    ...
    async fn merge_edges(
        ...
        for from in from_ids {
            if let Some(edge) = self.get_edge(kind, from, to_id).await.ok() {
                out.push(edge);
            }
        }
        Ok(out)
    ...
    async fn n_hop(
        ...
        Ok(crate::n_hop::NHopQuery::bfs_local(start_id, hop, &nodes, &edges))
    }
}

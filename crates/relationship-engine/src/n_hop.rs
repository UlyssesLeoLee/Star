...
use graph_core::repository::{GraphQueryResult, GraphRepository};
...
impl NHopQuery {
    ...
    pub async fn execute<R: GraphRepository + ?Sized>(
        ...
        let raw: GraphQueryResult = repo.n_hop(start_id, hop, direction, edge_kinds).await?;
        Ok(NHopResult {
            start_id,
            hop,
            nodes: raw.nodes,
        ...
        for edge in all_edges {
            match edge {
                ...
                EdgePayload::DerivedFrom(e) => {
                    adj.entry(e.from).or_default().push((e.to, edge));
                }
                EdgePayload::ConflictsWith(e) => {
                    adj.entry(e.from).or_default().push((e.to, edge));
                    adj.entry(e.to).or_default().push((e.from, edge));
                }
        ...
        while let Some((cur, depth)) = queue.pop_front() {
            ...
            if let Some(node) = all_nodes.iter().find(|n| {
                let id_opt: Option<uuid::Uuid> = match n {
                    NodePayload::Worktree(nn) => Some(nn.id),
                    NodePayload::AgentSession(nn) => Some(nn.id),
                    _ => None,
                };
                id_opt == Some(cur)
            }) {
...
mod tests {
    ...
    fn edge_query_graph_n_hop_1_2_3() {
        ...
        let edges = vec![derived_edge(a, b), derived_edge(b, c), derived_edge(c, d)];

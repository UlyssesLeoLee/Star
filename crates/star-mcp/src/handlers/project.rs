...
impl Resource for ProjectHandler {
    ...
    async fn read(&self, id: &str) -> Result<Option<Self::Data>, ResourceError> {
        ...
        let actor = ActorContext::nil_actor_with_tenant(tenant_id.0).with_role("project_admin");
        match svc
            .get_project(
                GetProjectQuery {
                    tenant_id,
...
mod tests {
    ...
    async fn read_real_project_roundtrip() {
        ...
        let actor = ActorContext::nil_actor_with_tenant(tid).with_role("project_admin");

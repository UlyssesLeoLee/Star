...
pub fn apply_session(
    ...
    // 守门 #7 首例 override:`std::process::Command::pre_exec` 是 `unsafe fn`
    // (closure 在 fork 后 exec 前运行,此上下文大多数 libc 调用都属 unsafe)。
    // closure 体只调 `nix::unistd::setsid()`(safe wrapper),无 AS-safety 问题。
    #[allow(unsafe_code)]
    unsafe {
        use std::os::unix::process::CommandExt;
        cmd.as_std_mut().pre_exec(|| {
            nix::unistd::setsid().map_err(std::io::Error::from_raw_os_error)
        });
    }
...
impl UnixSessionRegistry {
    ...
    fn init_schema(&self) -> Result<(), UnixSessionError> {
        let conn = self.conn.lock().expect("unix session registry mutex poisoned");
        conn.execute_batch(
            r#"

//! 集群外 Runtime 服务器侧 Registry / Port
//!
//! **crate**: `domain-local-runtime`
//! **上游 spec**: docs/specs/domain-local-runtime-spec.md §23 Local Runtime
//! **基本设计**: docs/basic-design.md §4.6 / §23 / §1.1 LocalRuntime 子图
...
//! ## 关键引用
//!
//! Local Daemon 是集群外独立进程,本 crate 仅服务器侧 Port(§4.6.1,F-07);9 种白名单命令(§4.6.2)
...
/// Runtime (聚合根 / 实体)
///
/// 来源: docs/data-design.md §4.25 (`local_runtime` schema)
///
...
/// **LocalRuntimeCommandPort**(命令端口)
///
/// 来源: docs/api-design.md §3.24 (Runtime Registry / Command / Observation)
///
/// **骨架阶段**: 仅方法签名,无 body 实现。Phase 2 在
/// `crates/infrastructure/<adapter>.rs` 中提供 SQLx / NATS / SCM Adapter 实现。
#[async_trait]
pub trait LocalRuntimeCommandPort: Send + Sync {
    async fn register_runtime(
        &self,
        cmd: RegisterRuntimeCommand,
        actor: ActorContext,
    ) -> Result<RuntimeId, LocalRuntimeError>;
    async fn send_command(
        &self,
        cmd: SendRuntimeCommand,
        actor: ActorContext,
    ) -> Result<RuntimeCommandId, LocalRuntimeError>;
...
/// **LocalRuntimeQueryPort**(查询端口)
///
/// 来源: docs/api-design.md §3.24 (Runtime Registry / Command / Observation)
#[async_trait]
pub trait LocalRuntimeQueryPort: Send + Sync {
    ...
    async fn list_observations(
        ...
        id: RuntimeId,
        viewer: ActorContext,
    ) -> Result<Vec<RuntimeObservation>, LocalRuntimeError>;
    async fn list_pending_commands(
        &self,
        id: RuntimeId,
        viewer: ActorContext,
    ) -> Result<Vec<RuntimeCommand>, LocalRuntimeError>;
}
...
/// Domain Event: `star.events.local_runtime.runtime.registered.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocalRuntime01Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.local_runtime.command.sent.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocalRuntime02Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.local_runtime.observation.received.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocalRuntime03Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
pub type RuntimeCommandId = Uuid;
pub type RuntimeId = Uuid;
pub type RuntimeObservationId = Uuid;

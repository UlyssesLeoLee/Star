// SPDX-License-Identifier: MIT OR Apache-2.0
//! RateLimitLayer stub (per spec §1.4)
//!
//! MVP 阶段: no-op (per OPT-NEXT-06-code-stub §3.1 3 中间件 stub).
//! P2 阶段 worker 子代理实装:
//! - 60 req/min per key 默认 (per spec §1.4)
//! - 1000 req/hour per key 默认
//! - burst 2x token bucket
//! - 超 → 429 Too Many Requests + `Retry-After` header
//! - MVP in-memory (单实例), Phase 2+ Redis 集群 (per spec §6 G-06)
//!
//! v0 phase 2 stub 标记 (per OPT-WORKER-01 模式): tracing 标注 `feature=RateLimitLayer`.

use axum::{extract::Request, middleware::Next, response::Response};

/// 限流中间件 stub — 当前 no-op
///
/// v0 phase 2 实装标记 (per OPT-NEXT-06-code-stub §3.1 3 中间件):
/// - 现状: pass-through, 不限流
/// - P2 阶段: 派 worker 子代理实装 token bucket (per spec §1.4)
pub async fn rate_limit_layer_stub(req: Request, next: Next) -> Response {
    tracing::debug!(
        feature = "RateLimitLayer",
        suggestion = "Phase M+ worker delegation per AGENTS.md §4 #20",
        method = %req.method(),
        uri = %req.uri(),
        "RateLimitLayer stub: pass-through (P2 phase real impl pending, 60 req/min + token bucket)"
    );
    next.run(req).await
}

#[cfg(test)]
mod tests {
    use super::*;
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use tower::ServiceExt;

    /// RateLimitLayer 中间件 pass-through 验证 (per OPT-NEXT-06-code-stub §3.1)
    #[tokio::test]
    async fn rate_limit_layer_passes_through() {
        use axum::{routing::get, Router};
        async fn hello() -> &'static str {
            "ok"
        }
        let app = Router::new()
            .route("/test", get(hello))
            .layer(axum::middleware::from_fn(rate_limit_layer_stub));
        let resp = app
            .oneshot(Request::builder().uri("/test").body(Body::empty()).unwrap())
            .await
            .unwrap();
        // RateLimitLayer pass-through → handler 返回 200
        assert_eq!(resp.status(), StatusCode::OK);
    }
}

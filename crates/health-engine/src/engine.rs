...
pub trait HealthEngine: Send + Sync {
    ...
    async fn recommend(
        &self,
        worktree_id: WorktreeId,
    ) -> Result<Recommendation, HealthError>;
}
...
mod tests {
    ...
    async fn engine_compute_then_get() {
        ...
        let score = engine.compute(id, input, HealthContext::default()).await.unwrap();
    ...
    async fn engine_recommend_after_compute() {
        ...
        engine.compute(id, input, HealthContext::default()).await.unwrap();
        let rec = engine.recommend(id).await.unwrap();
        assert!(rec.action.contains("Sync") || rec.action.contains("Main") || !rec.reason.is_empty());
    }

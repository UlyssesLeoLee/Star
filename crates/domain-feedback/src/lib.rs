...
//! **上游 spec**: docs/specs/domain-feedback-spec.md §25 Feedback Model
//! **基本设计**: docs/basic-design.md §2.1 / §4.3
//! **数据设计**: docs/data-design.md §4.5 (`feedback` schema)
//! **API 设计**: docs/api-design.md §3.8 (Feedback CRUD + Resolution)
//!
//! ## 职责
//!
...
//! ## 关键引用
//!
//! Feedback 6 状态(§7.5);覆盖 WorkItem/Diff Hunk 全粒度(§25.1)
...
/// Feedback (聚合根 / 实体)
///
/// 来源: docs/data-design.md §4.5 (`feedback` schema)
///
...
/// **FeedbackCommandPort**(命令端口)
///
/// 来源: docs/api-design.md §3.8 (Feedback CRUD + Resolution)
///
...
pub trait FeedbackCommandPort: Send + Sync {
    ...
    async fn resolve_feedback(
        &self,
        cmd: ResolveFeedbackCommand,
        actor: ActorContext,
    ) -> Result<FeedbackResolution, FeedbackError>;
    async fn link_to_validation(
        &self,
        cmd: LinkFeedbackValidationCommand,
        actor: ActorContext,
    ) -> Result<Feedback, FeedbackError>;
    async fn bulk_resolve(
        &self,
        cmd: BulkResolveFeedbackCommand,
        actor: ActorContext,
    ) -> Result<BulkResult, FeedbackError>;
}


/// **FeedbackQueryPort**(查询端口)
///
/// 来源: docs/api-design.md §3.8 (Feedback CRUD + Resolution)
#[async_trait]
...
/// Domain Event: `star.events.feedback.feedback.submitted.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Feedback01Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.feedback.feedback.resolved.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Feedback02Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
pub type FeedbackResolutionId = Uuid;
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BulkResolveFeedbackCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LinkFeedbackValidationCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ListFeedbackByTargetQuery {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ResolveFeedbackCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,

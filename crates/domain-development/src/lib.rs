...
//! **上游 spec**: docs/specs/domain-development-spec.md §20-21 Development Domain
//! **基本设计**: docs/basic-design.md §2.1 / §4.9.5 / §4.10.4
//! **数据设计**: docs/data-design.md §4.9 (`development` schema)
//! **API 设计**: docs/api-design.md §3.12 (ChangeSet / Link / SymbolIndex)
//!
//! ## 职责
//!
...
//! ## 关键引用
//!
//! Development Context(§20)合并入本 crate(F-02 修复)
...
/// DevelopmentExecution (聚合根 / 实体)
///
/// 来源: docs/data-design.md §4.9 (`development` schema)
///
...
/// **DevelopmentCommandPort**(命令端口)
///
/// 来源: docs/api-design.md §3.12 (ChangeSet / Link / SymbolIndex)
///
...
pub trait DevelopmentCommandPort: Send + Sync {
    ...
    async fn create_link(
        ...
        actor: ActorContext,
    ) -> Result<LinkId, DevelopmentError>;
    async fn register_symbol_index(
        &self,
        cmd: RegisterSymbolIndexCommand,
        actor: ActorContext,
    ) -> Result<SymbolIndex, DevelopmentError>;
}


/// **DevelopmentQueryPort**(查询端口)
///
/// 来源: docs/api-design.md §3.12 (ChangeSet / Link / SymbolIndex)
#[async_trait]
...
/// Domain Event: `star.events.development.execution.created.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Development01Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.development.change_set.recorded.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Development02Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.development.link.created.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Development03Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
pub type DevelopmentContextId = Uuid;
pub type DevelopmentExecutionId = Uuid;
pub type ExecutionId = Uuid;
pub type LinkId = Uuid;
pub type RepositoryContextId = Uuid;
pub type RepositoryId = Uuid;
pub type SymbolIndexId = Uuid;
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RecordChangeSetCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RegisterSymbolIndexCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,

...
// =====================================================================
// P0-4 infrastructure adapter 映射 (per WBS §14.15 P0-4 收官, 跟 P0-2/3 同模式)
// 1 个 InfrastructureError 5-variant → ApiError 6-field struct From impl
// =====================================================================

impl From<infrastructure::InfrastructureError> for ApiError {
    fn from(e: infrastructure::InfrastructureError) -> Self {
        use infrastructure::InfrastructureError::*;
        match e {
            NotFound(_) => ApiError::not_found("infrastructure"),
            InvalidState(msg) => ApiError::invalid_state(msg, "Check infrastructure adapter state"),
            PermissionDenied => ApiError::permission_denied("infrastructure: permission denied"),
...
mod tests {
    ...
    // 1 个 From impl 5-variant → ApiError 6-field struct 验证
    // -----------------------------------------------------------------

    #[test]
    fn infrastructure_error_not_found_maps_to_api_not_found() {
        use infrastructure::InfrastructureError;
    ...
    #[test]
    fn infrastructure_error_conflict_maps_to_api_conflict() {
        use infrastructure::InfrastructureError;
    ...
    #[test]
    fn infrastructure_error_internal_maps_to_api_internal() {
        use infrastructure::InfrastructureError;
        let e = InfrastructureError::Internal("connection lost".to_string());
        let api: ApiError = e.into();
        assert_eq!(api.code, "INTERNAL");
        assert_eq!(api.source_kind, "internal");
        assert!(api.retriable, "internal errors should be retriable");
    }
}

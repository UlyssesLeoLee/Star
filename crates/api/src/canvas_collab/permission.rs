...
//! `crates/api/src/canvas_collab/permission.rs` — RLS 13 类 + 6 角色 + 3 档权限 (View/Comment/Edit) 守门 (per DD-AGENT §3.3 + §4.14 + 守门 #13).
//!
//! 跟 `crates/api/src/agent/permission.rs` 同形, 但**额外**支持 3 档权限
//! (View/Comment/Edit) 强制 (per A12.7, BFF middleware 强制 per 9/1 13:03+13:05 JST
//! envoy 偏好). API tier 这边提供 5 守门 helpers:
//! 1. `check_tenant` — RLS 13 类 tenant 校验
//! 2. `require_role` — 单角色守门
//! 3. `require_any_role` — 联合角色守门
//! 4. `require` — 3 档权限等级守门 (View/Comment/Edit, per A12.7 派生)
//! 5. `is_platform_admin` — 跨租户管理员识别
//!
...
/// 6 类角色常量 (per 守门 #14 v2 + DD §3.3 派生).
pub mod role {
...
/// API tier 这边独立定义避免 cross-tier 紧耦合 (per 守门 #9 #1 禁跨层 leak).
...
#[derive(Debug, Clone)]
pub struct CanvasCollabPermission {
...
impl CanvasCollabPermission {
    ...
    /// **Helper 4**: 角色守门: 调用方必须持有 `role` (or `System`/`Admin`).
...
mod tests {
    ...
    fn check_tenant_accepts_same_tenant() {
        let p = CanvasCollabPermission::new(tenant(), RoleSet::new(), PermissionLevel::View);
        assert!(p.check_tenant(tenant()).is_ok());
    }

    #[test]
    fn require_edit_succeeds_with_edit_level() {
        let p = CanvasCollabPermission::new(tenant(), RoleSet::new(), PermissionLevel::Edit);
    ...
    fn require_edit_succeeds_for_platform_admin() {
        ...
        let p = CanvasCollabPermission::new(tenant(), roles, PermissionLevel::View);
        assert!(p.require(PermissionLevel::Edit).is_ok());
    }
}

...
//! `crates/api/src/agent/permission.rs` — RLS 13 类 + 6 角色权限守门 (per DD-AGENT §3.3 + 守门 #13).
//!
//! 跟 `crates/api/src/arg/permission.rs` 251 lines 同形, 但重命名为 `AgentPermission` 区分
//! 5 域 (ARG vs AGENT 跨域守门). 守门 #13 (W/T/M 横展) + 守门 #14 (5 域 Lead) 要求每个写路径
//! 都必须显式校验 tenant_id, 防止跨租户数据泄露.
//!
//! 6 角色 (per 守门 #14 v2 拍板 + 5 域 Lead CONTENT 4 维) — 跟 arg/permission.rs 同形:
//! - `Lead` / `AgentOwner` / `Viewer` / `Editor` / `Admin` / `System`
//!
//! 5 守门 helpers (per brief §2.3):
...
impl AgentPermission {
    ...
    /// **Helper 5**: 6 类角色联合守门: 调用方至少持有其中之一.
...
mod tests {
    ...
    fn check_tenant_accepts_same_tenant() {
        let p = AgentPermission::new(tenant(), RoleSet::new());
    ...
    fn admin_can_cross_tenant() {
        ...
        let p = AgentPermission::new(tenant(), roles);

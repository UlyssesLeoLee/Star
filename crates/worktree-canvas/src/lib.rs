...
//! ## 架构
//!
//! 实际实现为 **14 个 sibling crate** (`crates/graph-core/` .. `crates/worktree-shared-dir/`),
//! 本 crate 作为 **facade / 聚合层** re-export 14 个子 crate 的公开 API,
//! 提供单 import 入口 (`worktree-canvas::*`) 而非 14 个独立 crate 路径.
//!
//! ## 修正背景 (per ULYS-57 自审整体审查 2026-09-19 v1.2 → ULYS-158 + ULYS-177 合并 2026-09-22 v1.3)
//!
//! IMPL-PLAN §0.1 + SPEC §1.2 + DD §1.1 均声明 `crates/worktree-canvas/` 为
//! 14 crate 第一位. v1.2 (2026-09-19) M-1 修正补建 facade + 13 个 sibling crate.
//! v1.3 (2026-09-22) 加 worktree-shared-dir (per ULYS-158 + ULYS-177 合并,
//! 9/22 D-Boy 路径 C 自决: ConfigSource 走 FileBackedConfigSource, 不依赖 Multica CLI).
//!
//! ## 模块清单 (per DD §24 + IMPL-PLAN §1.2 + 9/22 升版)
//!
...
//! | `worktree-service` | 8 状态机 + ahead/behind + dirty + Provisioning 过渡态 | DD §12 | INV-WC-01/02/03/08/09 |
//! | `worktree-shared-dir` | SharedDirResolver + 3 Source (per-user + workspace + FileBackedConfigSource) + 5 MountStrategy + Priority 1-3 | DD §12/§24 + FR-ORCA-005..011 | — |
...
//! - #3 docs 同步 (per IMPL-PLAN §0.1 v1.2 → v1.3 修正)
...
pub use worktree_shared_dir;

#[cfg(test)]
mod tests {
    /// Smoke test: 所有 14 个子 crate 都能通过 facade 访问.
    /// 不做深度测试, 仅验证 re-export 不破坏编译 + 14 个子 crate 都注册到 workspace.
    ///
    /// 守门 #1 v25: cargo test -p worktree-canvas --lib 实证.
    ///
    /// 测试设计: 通过 `pub use` 引入的 14 个子 crate 都在模块作用域里.
    /// 仅引用 crate 自身的 module path (不需要任何子 crate 暴露任何 API),
    /// 触发 Rust 解析每个 crate root. 14 个子 crate 任意一个不可达时本测试编译失败.
    #[test]
    fn facade_re_exports_resolve() {
        // 14 个子 crate 都通过 `pub use` 引入, 它们的名字 (`graph_core` .. `worktree_shared_dir`)
        ...
        // 已经足够证明路径可达.
        //
        // 显式 use 让所有 14 个名字都进入作用域, 触发 rustc 的 unused 检查:
        ...
        use {
            ...
            crate::risk_engine, crate::worktree_service, crate::worktree_shared_dir,
        };
        // 通过 module_path!() 触发模块解析 (返回当前测试模块路径).
        // 14 个子 crate 已通过 `pub use` 在 crate root 暴露,
        // 这里仅占位证明测试可执行.

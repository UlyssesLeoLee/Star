...
//! **上游 spec**: docs/specs/domain-comment-spec.md §13 Comment / @ Mention / Attachment
//! **基本设计**: docs/basic-design.md §2.1 / §4.10.3
//! **数据设计**: docs/data-design.md §4.14 (`comment` schema)
//! **API 设计**: docs/api-design.md §3.17 (Comment / Mention / Attachment)
//!
//! ## 职责
//!
...
/// Comment (聚合根 / 实体)
///
/// 来源: docs/data-design.md §4.14 (`comment` schema)
///
...
/// **CommentCommandPort**(命令端口)
///
/// 来源: docs/api-design.md §3.17 (Comment / Mention / Attachment)
///
...
pub trait CommentQueryPort: Send + Sync {
    ...
    async fn list_mentions(
        ...
        id: UserId,
        viewer: ActorContext,
    ) -> Result<Vec<Mention>, CommentError>;
}
...
/// Domain Event: `star.events.comment.comment.created.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Comment01Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.comment.mention.created.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Comment02Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,

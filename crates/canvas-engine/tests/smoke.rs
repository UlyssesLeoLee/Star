//! canvas-engine lib tests
//!
//! 守门 #1 v25 cargo test 单 crate 实证 + 守门 #11 缺标比错标.

#[test]
fn service_metadata_is_skeleton() {
    let m = canvas_engine::ServiceMetadata::CANVAS_ENGINE;
    assert_eq!(m.name, "canvas-engine");
    assert_eq!(m.http_port, 8080);
    assert_eq!(m.phase, canvas_engine::Phase::Skeleton);
}
...
fn phase_serializes_to_snake_case() {
    let p = canvas_engine::Phase::Skeleton;
    let s = serde_json::to_string(&p).expect("serialize Phase");
...
#[tokio::test]
async fn router_version_returns_json_with_service_name() {
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use tower::ServiceExt;
    let app = canvas_engine::router();
    ...
    assert_eq!(v["service"], "canvas-engine");

...
impl InMemoryWorktreeService {
    ...
    pub fn new() -> Self {
        Self::with_providers(Arc::new(DefaultHealthProvider), Arc::new(DefaultRiskProvider))
    }
...
impl WorktreeService for InMemoryWorktreeService {
    ...
    async fn update(
        ...
        if let Some(e) = update.event {
            ...
            wt.human_state = lifecycle_transition(wt.human_state, &e, &snap)
                .map_err(|err| ServiceError::invalid_transition(prev_state, format!("{:?}", e), "trace")
        ...
        self.record_observed(&updated, prev_health, prev_state).await;
        Ok(updated)
    }
    ...
    async fn compute_health(&self, id: WorktreeId) -> Result<u8, ServiceError> {
        ...
        let provider = provider.ok_or_else(|| ServiceError::new(
...
mod tests {
    ...
    async fn worktree_cleanup_batch_with_provenance() {
        ...
        let mut wt_old = svc
            .create(repo_id, "wt-old", None)
            .await
            .unwrap();
        let mut wt_recent = svc
            .create(repo_id, "wt-recent", None)
            .await
            .unwrap();
        ...
        let removed = svc
            .cleanup_batch(repo_id, 30, false)
            .await
            .unwrap();

...
//!
//! **crate**: `domain-agent`
//! **上游 spec**: docs/specs/domain-agent-spec.md §4.2 / §24 Agent 策略
//! **基本设计**: docs/basic-design.md §2.1 / §4.2 / §7 状态机
...
//! ## 关键引用
//!
//! AgentSession 14 状态(§7.4,已修复 F-08);1 Session → 1 Active Worktree
...
/// Agent (聚合根 / 实体)
///
/// 来源: docs/data-design.md §4.3 (`agent` schema)
///
...
/// **AgentCommandPort**(命令端口)
///
/// 来源: docs/api-design.md §3.7 (Agent CRUD + AgentSession 状态机)
///
...
pub trait AgentCommandPort: Send + Sync {
    ...
    async fn submit_tool_result(
        &self,
        cmd: SubmitToolResultCommand,
        actor: ActorContext,
    ) -> Result<AgentSession, AgentError>;
    async fn submit_feedback(
        &self,
        cmd: SubmitFeedbackToSessionCommand,
        actor: ActorContext,
    ) -> Result<AgentSession, AgentError>;
...
pub trait AgentQueryPort: Send + Sync {
    ...
    async fn list_agent_policy(
        &self,
        id: ProjectId,
        viewer: ActorContext,
    ) -> Result<Vec<AgentPolicy>, AgentError>;
}
...
/// Domain Event: `star.events.agent.agent_session.started.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Agent01Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.agent.agent_session.state_changed.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Agent02Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
/// Domain Event: `star.events.agent.agent_session.completed.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Agent03Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ListAgentSessionQuery {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubmitFeedbackToSessionCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubmitToolResultCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UpdateAgentPolicyCommand {
    /// 主键 UUID
    pub id: Uuid,
    /// 租户 ID(13 类对象必带,§6.1)
    pub tenant_id: Uuid,

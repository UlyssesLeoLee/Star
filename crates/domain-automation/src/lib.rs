...
//! **上游 spec**: docs/specs/domain-automation-spec.md §11 Automation
//! **基本设计**: docs/basic-design.md §2.1 / §4.10.5
//! **数据设计**: docs/data-design.md §4.19 (`automation` schema)
//! **API 设计**: docs/api-design.md §3.22 (Rule / Trigger / Action)
//!
//! ## 职责
//!
...
/// Rule (聚合根 / 实体)
///
/// 来源: docs/data-design.md §4.19 (`automation` schema)
///
...
/// **AutomationCommandPort**(命令端口)
///
/// 来源: docs/api-design.md §3.22 (Rule / Trigger / Action)
///
...
pub trait AutomationQueryPort: Send + Sync {
    async fn list_rules(
        ...
        id: ProjectId,
        viewer: ActorContext,
    ) -> Result<Vec<Rule>, AutomationError>;
    async fn get_rule(
        &self,
        id: RuleId,
        viewer: ActorContext,
...
/// Domain Event: `star.events.automation.rule.fired.v1`
///
/// 来源: docs/api-design.md §5 (CloudEvents 1.0)
///
/// **骨架阶段**: 仅占位字段,Phase 2 补充完整 Payload 字段。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Automation01Event {
    /// 事件唯一 ID(UUIDv7)
    pub event_id: Uuid,
    /// 租户 ID(必带)
    pub tenant_id: Uuid,

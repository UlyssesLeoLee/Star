...
)
{
    ...
    echo ""
    echo "- 缺口 #1: ~~k3s/ 仅 2 yaml, 缺 star-mock ConfigMap + Secret~~ → **已闭合 (v1.2)** k3s/star-mock-configmap.yaml + k3s/star-mock-secret.yaml 新增 (实测 §1 4 yaml, 守门 #5 占位符 allowlist OK)"
    echo "- 缺口 #2: port-forward 守护依赖 WSL2 + systemd user (Windows env 限制) — 跨 session 续"
    echo "- 缺口 #3: ST 层静态验证, 不连真 k3s (per 守门 #24 v2 G-5 mock 锁) — by design, 不闭合"
    echo ""
} > "$report"
echo "  [OK] $report 生成"
echo ""

...
- 缺口 #1: 16 MCP tool 仅覆盖 6, 缺 10 (per docs/reports/MCP-16-TOOL-100-COVERAGE-REPORT.md 已知)
- 缺口 #2: Agent Runtime L1 ECS 9 Archetype 仅 2 fixture (per AGENTS.md §3)
- 缺口 #3: Streamable HTTP 5xx 错误 + retry 完整 case 缺失 (per docs/test-design/TEST-DESIGN-STREAMABLE-HTTP-001.md)
- 缺口 #4: 5 域 fixture 复用 frontend/src/mocks/data/five-domain.ts (per README v0.2 §1.2)
- 缺口 #5: k3s/ 仅 2 yaml, 缺 star-mock ConfigMap + Secret (per regression-test-system.sh §6)
- 缺口 #6: ST 层静态验证, 不连真 k3s (per 守门 #24 v2 G-5 mock 锁)
- 缺口 #7: docs/ 回归报告 (本文件) 仅初次生成, 缺自动化 PR/CI 触发
...
4. v1.2 (跨 session 续): 接入 P5 DB W/T/M 100% 覆盖率强制校验
5. v1.2 (跨 session 续): 接入 frontend handlers-5d.test.ts (per §UT 5 域 fixture 检查)
6. v1.3 (跨 session 续): 接入 CI (per docs/runbook §7), 失败即停

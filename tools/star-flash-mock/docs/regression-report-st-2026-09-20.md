...
- 缺口 #1: k3s/ 仅 2 yaml, 缺 star-mock ConfigMap + Secret (envoy + 服务配置)
- 缺口 #2: port-forward 守护依赖 WSL2 + systemd user (Windows env 限制)
- 缺口 #3: ST 层静态验证, 不连真 k3s (per 守门 #24 v2 G-5 mock 锁)

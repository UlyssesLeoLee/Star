#!/usr/bin/env python3
"""
P3-D.5 实施计划 doc + WBS v0.86 + §4.32 + registry v0.17 落档脚本 (per 守门 #9 v19 Mavis 自驱 + 守门 #1 v19 批量改 1 commit 收官).

触发: 2026-09-10 19:18 JST Ulysses 拍板"根据详细设计制作spec实施计划，并加入wbs".
范围:
  1. docs/implementation-plans/CANVAS-IMPL-PLAN-001.md v0.1 落档 (基于 P3-D.5 3 份 DD)
  2. WBS v0.86 row 标 P3-D.6 实施计划
  3. §4.32 task card
  4. registry v0.17 row
"""
import sys
...
# ===== 1. 实施计划 doc content =====
IMPL_PLAN_PATH = REPO_ROOT / "docs/implementation-plans/CANVAS-IMPL-PLAN-001.md"

IMPL_PLAN_CONTENT = """# CANVAS-IMPL-PLAN-001
...
def main() -> int:
    ...
    if "**v0.86**" in wbs_content:
        print(f"  WBS: v0.86 already exists, skipping")
    else:
        if not wbs_content.endswith("\n"):
            wbs_content = wbs_content + "\n"
        new_wbs = wbs_content + WBS_V086_ROW + "\n"
        wbs_path.write_text(new_wbs, encoding="utf-8")
        print(f"  Appended v0.86 row ({len(WBS_V086_ROW)} chars)")

    # 3. Append §4.32 to automation-design.md
    ...
    if "### 4.32 P3-D.6 启动实装 spec 实施计划落档" in auto_content:
        print(f"  automation-design: §4.32 already exists, skipping")
    else:
        if not auto_content.endswith("\n"):
            auto_content = auto_content + "\n"
        new_auto = auto_content + SECTION_4_32 + "\n"
        auto_path.write_text(new_auto, encoding="utf-8")
        print(f"  Appended §4.32 ({len(SECTION_4_32)} chars)")

    # 4. Append v0.17 row to registry.md
    ...
    if "**v0.17**" in reg_content:
        print(f"  registry: v0.17 already exists, skipping")
    else:
        if not reg_content.endswith("\n"):
            reg_content = reg_content + "\n"
        new_reg = reg_content + V017_ROW + "\n"
        reg_path.write_text(new_reg, encoding="utf-8")
        print(f"  Appended v0.17 row ({len(V017_ROW)} chars)")

...
> **文档版本**: v5.5 (2026-09-08 21:40 JST, 强杀老 pod 重试 fail - 跟 v30 候选 a WSL 资源耗尽同源)
> **v5.4 → v5.5 强杀重试 fail (per Ulysses 21:35 JST 答 A)**: 21:35 JST Ulysses 答 A (推断 = 强杀老 pod 重试). Mavis 自驱: 21:36 `kubectl delete pod --all --force --grace-period=0` 强杀 ✅. 21:38 scale --replicas=1 触发. 21:40 新 pod 0/1 Pending 119s+, 仍然 "0/1 nodes are available: 1 node(s) didn't have free ports" (k3s scheduler 跟 v3.0 8:14 8:24 WSL host 半死同源). **v5.5 失败**: 强杀老 pod 修不通, WSL 端 cgroup 端口释放慢 + hostNetwork:true 跨 wsl 端 end-to-end 不可行. **老实回答 Ulysses 21:35 (per 守门 #11)**: 强杀也修不通, 跟 v30 候选 a WSL 资源耗尽同源. **Mavis 自驱推荐 (per 9/8 16:08 守门"必带推荐")**: **选 D = 回退 v5.2 3001 next start 跟 k3s 解耦** (接受两个独立服务, 不再跟 wsl 半死搏斗, 跟守门 #38 + 守门 v33 候选一致). 理由: v5.3 NodePort 配上了 + v5.4 hostNetwork 配上了 + k3s pod 2/2 Running 实证, k3s 内部 cluster 完全 work, 只是 WSL 端 → Windows 端 NAT 桥接不通. 跟 star 真实 frontend (3001 next start) 解耦后, 各自独立, 不冲突. 34→35 commit 链.
> **v5.3 → v5.4 hostNetwork 修法失败 (per Ulysses 21:22 JST "修复问题" + 21:23 JST 选 A)**: 21:23 JST Ulysses 拍板 A = "Deployment hostNetwork: true (推荐)". 21:25 Mavis 自驱改 envoy-deployment.yaml 加 `hostNetwork: true` + `dnsPolicy: ClusterFirstWithHostNet` + `hostPort: 8080/8443`. 21:27 `kubectl apply --server-side` 成功 (`serverside-applied` ✅). 21:28 `kubectl rollout restart deployment star-mock-envoy` 触发 reload. 21:30 老 pod Terminating 7m+, 新 pod 0/1 Pending 2m9s 卡死. **根因 (k3s scheduler 实证)**: `FailedScheduling: 0/1 nodes are available: 1 node(s) didn't have free ports for the requested pod ports` — 老 pod (hostNetwork:true 同样 8080/8443 hostPort) 还在 cgroup 端口占用, 新 pod 排不到. 21:32 `kubectl scale --replicas=0` 释放老 pod, 老 pod Terminating 9m16s 仍未释放 hostPort (跟 v3.0 8:14 8:24 实证 wsl host 半死同源). 21:35 `kubectl scale --replicas=2` 触发新 pod 拉, 但 2 个新 pod 0/1 Pending 53s+, 仍然 "didn't have free ports" 提示 (老 pod Terminating cgroup 端口未释放). **v5.4 失败**: hostNetwork + replicas 2 跟 k3s 1.36.4 + WSL2 端不兼容 (跟 v30 候选 (a) 实证 wsl 半死同源). 老实回答 Ulysses 21:22: "修复问题" 通过 Deployment hostNetwork 修法 不可行, 老 pod Terminating hostPort 8080 释放不出来, 新 pod 排不到. **后续**:
- 选 B 镜像模式 (WSL2 mirrored networkingMode) — 需重启 WSL 跟 Ulysses 必手动
- 选 D 回退 v5.2 (3001 next start 跟 k3s 解耦) — 接受两个独立服务
- 选 A 强杀老 pod (`kubectl delete pod --force --grace-period=0`) 后重试 — Mavis 试一次

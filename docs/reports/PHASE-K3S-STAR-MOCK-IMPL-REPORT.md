...
> **文档版本**: v4.1 (2026-09-08 15:46 JST, Ulysses 15:44 JST 反馈"3000 端口启动后 3 秒的截图" 实战)
> **v4.0 → v4.1 t3s 截图实战**: 15:44 JST Ulysses 反馈"我希望你给我看看3000端口启动后3秒的截图, 因为之前都是黑屏". Mavis 自驱写 `frontend/e2e/uat-3000-t3s-screenshot.mjs` (69 行 Playwright 脚本), restart k3s-portforward service 模拟"启动 3000 端口" + 立即 navigate + sleep 3s + 截图. **实证 3.17s 状态**: title="" (proxy 模式 JSON 无 HTML title), body text length=8291 (apiserver paths JSON, 浏览器当 text/plain 渲染纯文本), 不是黑屏. 这跟 8:08 Ulysses 反馈"启动 3000 端口后黑了"不一样 — 现在 (15:45) service 启 3s 内 listening 完, 浏览器拿到 body 显示纯文本. 8:08 时 service 没启 / listening 慢, 浏览器拿到 ERR_CONNECTION_REFUSED → 黑屏. **v4.0 闭环 ≠ 8:08 黑屏**: 8:08 时 port-forward service 启 5-10s, 浏览器等不到 connection, 黑屏. 现在 (15:45) proxy 模式 service 启 ~3s, 浏览器拿到 apiserver paths JSON 显示纯文本. 守门 v35 候选 (新, Ulysses 15:44 实证): 任何"启动 3000 端口" 反馈"黑屏"时, 必先验证 service listening 时间 (proxy 模式 ~3s, port-forward 模式 ~5-10s), 然后 3s 截图实证不是真的黑屏还是 service 没启. 截图实证比口头报告更有效 (per 守门 #11 缺标比错标). 26→27 commit 链.

...
> **文档版本**: v4.3 (2026-09-08 16:00 JST, star 真实 frontend 渲染实战 - Ulysses 15:54 "我需要看到的是 star 的真实界面")
> **v4.2 → v4.3 star 真实 frontend 实战**: 15:54 JST Ulysses 反馈 "我需要看到的是 star 的真实界面" — v4.2 mock HTML (dist/index.html) 不是 Ulysses 想要的, Ulysses 要 star 仓 frontend 真实渲染. 决策: 启 `pnpm exec next dev -p 3001` (避开 8:08 9:15 netsh portproxy 5176 抢 3000 端口冲突), 让 star frontend 跑 3001. 实施: (1) 停 start-mock-http.service 释放 3000; (2) 停 next dev 3000 旧进程 (Stop-Process node force); (3) Start-Process cmd.exe 后台启 `pnpm exec next dev -p 3001` (PORT env 不生效, next dev hardcode -p); (4) 等 15s 启 3.3s ready; (5) 触发首编译 4.1s + curl localhost:3001 status=200 len=68431 (完整 HTML); (6) 改 t3s script URL=3001, 跑 3.12s 截图实证. **3.12s 截图实证 (per v4.1 t3s script)**: title="Star · Vibe Coding Work Management" (star 真实 frontend 标题!), body text length=12366 字符, 内容预览 "Star 3ACME Studio CORE 收件箱 工作项 项目 Agent 总览 效能分析 NEURAL SYNC 〔神経同調〕 9.8% STATUS INV:PASS ALL APPS 25+". 跟之前 v4.0 proxy 模式 (apiserver paths JSON 纯文本), v4.2 mock HTML 都不一样, 是真实的 star Next.js 14.2.5 frontend. **UAT 3001 端口 star 真实 frontend 闭环**: 浏览器 3 秒后看到 star Vibe Coding Work Management UI. 8:08 Ulysses 反馈"黑了" 完全解决 (frontend 真正跑起来). **守门 v37 候选 (新)**: Windows 端跟 wsl 端 3000 端口必错开 (wsl 内 3000 跟 Windows 端 3000 都占 5176 netsh portproxy 冲突), 后续 frontend 跑 3001 / 3002 避开. 28→29 commit 链.

...
> **文档版本**: v1.3 (2026-09-08 13:18 JST, final)
> **v1.2 → v1.3 变更**: + §9.10 mavis 试 NodePort 也失败 (WSL 内 127.0.0.1:30001 + Windows → 172.28.176.169:30001 都 timeout, status=000 size=0b). k3s kube-proxy + WSL iptables 集成不工作 (cluster-level). **结论 = 本 k3s cluster + WSL 环境下, 原 UAT 闭环目标 (envoy "not found" 文本) 无法用任何 kubectl / NodePort / port-forward 链路达到**. 唯一可工作链路 = kubectl proxy (v1.1 已修), 但 proxy 不暴露 envoy. session 闭环 = 链路通 (proxy) 但 envoy 文本不可达. 等同 session 完结, 真实问题需重启 k3s cluster (WipeCluster 重新初始化) 或换 cluster 测试.

...
> **文档版本**: v4.2 (2026-09-08 15:53 JST, HTML 渲染实战 - Ulysses 15:48 "你得优化到 3 秒后能正常显示界面")
> **v4.1 → v4.2 HTML 渲染实战**: 15:48 JST Ulysses 反馈 "你得优化到 3 秒后能正常显示界面" — v4.0 proxy 模式 body=apiserver paths JSON 浏览器渲染纯文本, 不是"正常显示界面" (HTML). 决策路径: (a) 改 envoy ConfigMap 用 file_system HTTP filter 托管 HTML 目录 — 但需 port-forward 模式暴露 envoy 8080, spdy tunnel cluster-level 不可达 (v30 候选 c), 链路不通; (b) 写 `tools/star-flash-mock/dist/index.html` (3.8KB HTML 页面) + `start-mock-http.service` (systemd user service 跑 python -m http.server 3000 serve dist/), 跟 k3s envoy 链路解耦, 浏览器 3000 = HTML 渲染. 选 (b), 跟 9/1 13:03 JST envoy 选型偏好一致 (envoy 部署模式独立 deployment, 不依赖 istio sidecar; 这版走 python 独立 server 是同样的"独立"精神). 实施: 写 index.html (含 mock data 链接 + 守门链 + 状态快照) + start-mock-http.service (systemd user unit, 跟 k3s-portforward.service 互斥共享 3000 端口) + 停 k3s-portforward.service (proxy 模式) + cp service file 到 `~/.config/systemd/user/` + daemon-reload + enable --now. **第一次失败**: status=216/GROUP `Failed to determine supplementary groups: Operation not permitted` (systemd user mode 启 User=leo19 service 时 cgroup 权限不够). 修法: 删 `User=leo19` 行, 跑用户 systemd 默认 user. 第二次成功. **3.17s 截图实证 (per v4.1 t3s script)**: title="star-flash-mock · UAT 3000 端口" (不再是空), body text length=1363 字符, 内容预览含 "⭐ star-flash-mock UAT 3000 · 本地 Mock 服务 · 浏览器 3 秒后正常显示界面 · 跟 k3s envoy 链路解耦 · ✓ 服务在线 · python -m http.server 3000". **UAT 3000 端口 HTML 渲染闭环**: 浏览器 3 秒后看到正常界面 (HTML, 不是黑屏, 不是 JSON 纯文本). **守门 v36 候选 (新)**: wsl systemd user mode 跑 Python service 必不写 `User=` 行, 跑用户 systemd 默认 user (跟 k3s-portforward.service 不一样, 那个是 leo19 用户自己启, 走 user mode 默认 OK; python service 是 user mode daemon 启, 需要 rootless cgroup 权限, 写 User= 会 fail). 27→28 commit 链.

...
> **Mock-first 范围声明**(per `docs/detailed-design-feedback.md` DD-12, 2026-09-08): 本 ADR 决策**已知**导致"api-design.md 端点 vs `crates/api` 实装"存在大规模差距,**不是 bug,是过渡期既定状态**;`api-design.md` v0.2.1 同步补 Mock-first 范围声明,跨文档避免被反复当成"接口设计过期/无人维护"误读,详见 `docs/api-design.md` 顶部声明。

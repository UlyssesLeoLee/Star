...
| T1.5 | lint `warn` → `deny` (3 项) | T1 | 0.3M | T1.3 (先确定 crate 数) | 无 (强制不强制, 视情况) | ✅ 3/3 步完成 (`unreachable_pub` deny, commit `bef2d60`, 后续发现 31 处回归由 `850d71c` 修复; `rust_2018_idioms` deny, commit `d9f65b3`; `missing_docs` deny, 隔离 worktree `.worktrees/wt-t15-missing-docs`(分支 `rf001-t15-work`) 内逐 crate 补文档 + 逐 crate commit 收尾 domain-search/domain-board/star-treesitter/domain-dashboard/star-taskgraph/domain-ai/domain-cli/application/domain-workspace/domain-integration/domain-kms(另单独 flip 其自身 `Cargo.toml` `[lints.rust]`)/infrastructure, 全 workspace `cargo check --workspace --all-targets --keep-going` 复测 0 `missing_docs` 警告后根 `Cargo.toml` 三项 lint 均已 `deny`; 7 个既有 `#![allow(missing_docs)]` crate(`domain-feedback`/`domain-validation`/`star-api-rest`/`star-dispatcher`/`star-dto`/`star-sa`/`star-saga`)按既有决定视为 T1.5 范围外遗留债务, 未纳入本次测量基线, 未拆除其 `allow`) |
...
| **小计** | | | **~4.15-4.95M** | | | **5/12 完成 (T1.1/T1.2/T1.3/T1.4/T1.5) (per 2026-09-05 本 session 实测, T1.5 `missing_docs` 收尾)** |
...
**收尾记录 (2026-09-05, 隔离 worktree 内, 逐 crate 顺序处理完成)**:

- 延续"纠正后的执行方式": 每 crate 编辑完立即 `cargo check -p <crate> --all-targets` 复测 0 剩余 `missing_docs` 警告、`git diff` 差异安全性核对(仅新增 `///` 行 + 少量既有行内 `//` 注释改写为独立 `///` 行, 无业务逻辑/格式/测试改动)后, 逐 crate 单独 commit, 依次收尾: `domain-search`(64 项, `2f1b48f`)/`domain-board`(60 项, `63b74f6`)/`star-treesitter`(57 项, `7479070`)/`domain-dashboard`(51 项, `317fd32`)/`star-taskgraph`(50 项, `dba21c8`)/`domain-ai`(47 项, `81d9f9c`)/`domain-cli`(42 项, `152d088`)/`application`(21 项, `810a40a`)/`domain-workspace`(20 项, `b48e844`)/`domain-integration`(18 项, `45f4126`)/`domain-kms`(14 项, `8672a03`, 另单独把其自身 `Cargo.toml` 硬编码的 `[lints.rust] missing_docs` 从 `"warn"` 一并 flip 为 `"deny"`, 因该 crate 不继承 workspace 级 lints)/`infrastructure`(11 项, `5ebb9cd`)。
- 全 workspace 收尾验证: `cargo check --workspace --all-targets --keep-going` 复测 0 `missing_docs` 警告(确认此前"76 file / 1683 行"清单已被以上 crate 完全覆盖, 无遗漏)。据此把根 `Cargo.toml` `[workspace.lints.rust]` 的 `missing_docs` 从 `"warn"` flip 为 `"deny"`, 再跑一次 `cargo check --workspace --all-targets --keep-going` 确认 0 error/0 `missing_docs` 警告(仅剩余与 T1.5 无关的既有 `unused_imports`/`dead_code`/`unused_mut` 类警告, 判定为范围外债务未处理)。
- `cargo fmt --all -- --check` 发现 1 处既有格式差异(`domain-planning/src/whatif.rs` 的 `Confidence` enum 变体行内注释对齐空格), 核实 `main` 分支上该对齐写法本就存在(先于任何 T1.5 改动), 判定为范围外既有格式债务, 未在本次改动中一并重排(避免超出"只加文档"的改动边界)。
- 范围决定(延续): 7 个既有 `#![allow(missing_docs)]` 的 crate(`domain-feedback`/`domain-validation`/`star-api-rest`/`star-dispatcher`/`star-dto`/`star-sa`/`star-saga`)保留原样, 不计入本次 T1.5 `missing_docs` 测量基线, 也未拆除其 `allow` 属性(如需清零这批遗留债务应作为独立后续任务, 而非并入本次 lint flip)。
- **验收门复核(修正首次验证的盲区后重跑)**: 首次验证用 `cargo check --keep-going --message-format=short | grep -i "missing documentation"` + `grep -c "^error"` 判空, 但 `--message-format=short` 的诊断行是 `path:line:col: error: ...` 不从行首开始, `grep -c "^error"` 结构性照不到任何真实报错, 且当时管道里混了 `tee | tail`, 退出码取的是 `tail` 而非 `cargo`。改用 `grep -cE ": error(\[|:)|^error"` 精确匹配 + 单独取 `cargo` 本身的 `$?` 重跑: `cargo check --workspace --all-targets --keep-going --message-format=short`, 结果 `EXIT=0`, 真实 error 匹配数 `0`, `missing_docs`/`missing documentation` 匹配数 `0`, 剩余 130 条警告全部是 `unused_imports`/`unused variable`/`field ... never read` 类既有债务, 与 T1.5 无关。另确认 52 个 workspace member 里仅 `domain-kms` 有自己硬编码的 `[lints.rust]` 表(已在上一步单独 flip), 其余 51 个均为 `lints.workspace = true`, 根 `Cargo.toml` 的 flip 对它们全部生效, 无遗漏 crate。
- **`cargo test --workspace --lib --no-fail-fast` 复测**: 默认并行度下 50 个 test 二进制全部 `Running unittests`, 全部 `test result: ok`, 0 failed(含 `star-cache`——此前诊断过的 `redis_backend::tests::from_env_ok`/`from_env_unset` 共享 `REDIS_URL` 环境变量竞态本次未复现, 属既有 flaky 债务, 非本次改动引入, 不需要也未在本次修复)。
- **`cargo clippy --workspace --lib` 复测**: `EXIT=0`, 0 error, 198 warning。抽查其中 4 条 `empty line doc comment`(`api`/`application`/`infrastructure` 各 crate 的 "命令 / 查询 / 跨 crate 类型占位结构" 占位注释块, 空行分隔多段 `///`)经 `git log -S` 溯源确认该文本模式已存在于最初的 scaffold commit `89a14d2`(早于任何 T1.5 改动), 本次 `missing_docs` 补文档只在结构体/字段/方法上方逐行加 `///`, 未触碰这段既有占位头注释, 判定为范围外既有债务。198 条 warning 未与 T1.5 启动前的 baseline 逐条比对数字, 但抽查确认新增文档未引入新的 clippy 报错类别(0 error 不变), 结论: clippy 状态对 T1.5 验收无阻塞。
- 未推送 / 未合并: 本次全部改动停留在隔离 worktree `.worktrees/wt-t15-missing-docs`(分支 `rf001-t15-work`), 未 push `origin`, 未 merge 回共享 `main`——按当前自动化 session 的安全边界, 该分支即为本任务的有效终态, 合并决策留给 Ulysses。
...
| v0.6 | 2026-09-05 | 本 session AI | T1.5 `missing_docs` 收尾: 隔离 worktree 内逐 crate 补文档 + 逐 crate commit 完成剩余 `domain-search`/`domain-board`/`star-treesitter`/`domain-dashboard`/`star-taskgraph`/`domain-ai`/`domain-cli`/`application`/`domain-workspace`/`domain-integration`/`domain-kms`/`infrastructure` 12 crate; 全 workspace `cargo check --workspace --all-targets --keep-going` 复测 0 `missing_docs` 警告后根 `Cargo.toml` `missing_docs` flip `"warn"`→`"deny"`(至此 T1.5 三项 lint 全部 `deny`); 修正首次验收检查的 grep 盲区后重跑确认 `EXIT=0`/0 error/0 missing_docs; `cargo test --workspace --lib --no-fail-fast` 50 个 test 二进制全过(含 `star-cache` 既有 flaky 竞态本次未复现); `cargo clippy --workspace --lib` 0 error/198 warning(含 4 条 pre-existing `empty line doc comment`, 溯源至 scaffold commit `89a14d2`, 与本次改动无关); T1.5 状态改为 ✅ 3/3 完成, 小计从 4/12+1/12 部分完成改为 5/12 完成; 本次改动停留在 `rf001-t15-work` 分支, 未推送 `origin`, 未合并 `main`, 详见 §T1.5 明细段"收尾记录" |

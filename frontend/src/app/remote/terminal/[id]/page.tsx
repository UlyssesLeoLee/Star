...
import { XtermViewer } from "@/components/remote/XtermViewer";
  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] md:h-[calc(100vh-4rem)] flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-line bg-bg-soft/60">
        ...
        <span className="text-sm font-semibold text-ink">远程终端 · {runtime.hostname}</span>
      </div>
      <div className="flex-1 min-h-0">
        <XtermViewer runtimeId={id} hostname={runtime.hostname} />
      </div>
    </div>
  );
}

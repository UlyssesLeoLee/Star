...
const CATEGORIES: Array<{ id: "all" | ModuleCategory; label: string; tag: string }> = [
  { id: "all", label: "全部模块", tag: "ALL" },
  { id: "core", label: "核心工作区", tag: "CORE" },
  { id: "work", label: "工作管理", tag: "WORK" },
  { id: "agent", label: "Agent & Worktree", tag: "AGENT" },
  { id: "integration", label: "集成与网关", tag: "INTEG" },
  { id: "system", label: "系统与安全", tag: "SYS" },
];
...
export function AppMatrixDrawer() {
  ...
  const [selectedCat, setSelectedCat] = useState<"all" | ModuleCategory>("all");
      ...
      >
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-ink-dim font-medium">
                默认仅展示核心功能，按需将任何模块一键钉选至左侧纵向或顶部横向导航。
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-3.5 border-b border-line bg-bg-soft/30 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="快速检索 25+ 业务模块、简码代码 (例: KB, SCM, WT, AUD)..."
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredModules.map((m) => {
            const Icon = m.icon;
            const inSidebar = sidebarItemIds.includes(m.id);
            const inHeader = headerTabIds.includes(m.id);
              ...
              >
                ...
                <div className="border-t border-line/50 pt-3 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      onClick={() => toggleSidebarItem(m.id)}
                      ...
                      title={inSidebar ? "从左侧导航移除" : "钉选到左侧导航"}
                    >
                    ...
                    <button
                      ...
                      title={inHeader ? "从顶部标签移除" : "钉选到顶部标签"}
                    >
        ...
        <div className="px-6 py-3.5 border-t border-line bg-bg-soft/50 flex items-center justify-between text-xs text-ink-mute font-mono">
          <span className="flex items-center gap-3">
            <span>左侧已钉选: <strong className="text-accent font-bold">{sidebarItemIds.length}</strong></span>
            <span className="text-line">|</span>
            <span>顶部已钉选: <strong className="text-purple-400 font-bold">{headerTabIds.length}</strong></span>
          </span>
          <button
            type="button"
            onClick={close}

...
//! `bff/src/collaboration/audit.rs` — 100% audit middleware for BFF canvas collaboration (per 守门 #13 d + DD-AGENT §4.14 + DD-001 C-26 + brief §2.7).
//!
//! Per brief §2.7: 6 字段 `CollaborationAuditEvent` + `CollaborationAudit` sink
...
//!
//! 跟 `crates/api/src/canvas_collab/audit.rs` 5 字段不同, BFF 这边多 1 个
//! `metadata: serde_json::Value` 字段 (per A12.5 @ 提醒 + payload snapshot 派生).
//! BFF tier 独立定义 (跟 API tier 平级 0 反向依赖, per 守门 #1 禁回溯叙事 + 守门 #11 缺标比错标).
//!
//! 守门合规 (per 守门 #1 v25 + 守门 #7 + 守门 #13 d + 守门 #14 v2):
//! - 0 `unsafe` blocks (守门 #7 `unsafe_code = "forbid"`).
//! - 100% 写路径过 `CollaborationAudit::write_event` (per 守门 #13 d).
...
/// 6 字段 `CollaborationAuditEvent` (per brief §2.7 + DD-AGENT §4.14 + DD-001 C-26 派生).
///
/// 跟 `crates/api/src/canvas_collab/audit.rs` `CanvasCollabAuditEvent` 同形,
/// BFF tier 独立定义避免 cross-tier 紧耦合 (per 守门 #9 #1 禁跨层 leak).
...
pub struct CollaborationAuditEvent {
    ...
    pub actor_id: Uuid,
    /// 操作动作 (e.g. "create_element" / "update_element" / "add_comment" / "set_follow" / "get_presence" / "delete_element").
    pub action: String,
...
impl CollaborationAuditEvent {
    /// 构造一个新的 `CollaborationAuditEvent`.
    pub fn new(
        canvas_id: Uuid,
        actor_id: Uuid,
...
impl CollaborationAudit {
    ...
    pub async fn write_event(&self, event: CollaborationAuditEvent) -> Result<(), ApiError> {
        ...
        if event.actor_id.is_nil() {
            return Err(ApiError::BadRequest("audit event actor_id is nil".into()));
        }
...
mod tests {
    ...
    fn audit_event_new_fills_id_and_at() {
        ...
        let e = CollaborationAuditEvent::new(
            canvas_id,
            actor_id,
            "create_element",
            serde_json::json!({}),
    ...
    async fn write_event_accepts_well_formed_event() {
        let audit = CollaborationAudit::new();
        let event = CollaborationAuditEvent::new(
            Uuid::new_v4(),
            Uuid::new_v4(),
            "add_comment",
    ...
    #[tokio::test]
    async fn write_event_rejects_nil_actor_id() {
        let audit = CollaborationAudit::new();
        let event = CollaborationAuditEvent::new(
            Uuid::new_v4(),
            Uuid::nil(),
